...
for _, row in merged.iterrows():
    if row.geometry:
        ...
        share_val = f"<b>{int(row[share_col_name])}%</b><br><span style='font-size:10px;'>({int(row['Market_Size'])} MT)</span>" if pd.notna(row[share_col_name]) else f"0% <br> ({int(row['Market_Size'])} MT)"
        # 1. District Name
